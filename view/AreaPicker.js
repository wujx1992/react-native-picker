...
class AreaPicker extends BaseDialog {
  ...
  // }
  getAreaData() {
    let area = this.props.areaJson.provinces;
    let data = [];
    let len = area.length;
    for (let i = 0; i < len; i++) {
      let city = [];
      for (let j = 0, cityLen = area[i]['cities'].length; j < cityLen; j++) {
        let _city = {};
        const districts = area[i]['cities'][j]['districts'];
        let districtNames = [];
        districts.map((district) => {
          districtNames.push(district.name);
        })
        _city[area[i]['cities'][j]['name']] = districtNames;
        city.push(_city);
      }
      let _data = {};
      _data[area[i]['name']] = city;
    ...
    return this.state.pickerData.map((item, pickerId) => {
      ...
      if (item && length > 0) {
        return <PickerView
          ...
          key={'picker' + pickerId}
          list={item}
          onPickerSelected={(toValue,selectedIndex) => {
            this.props.selectedValue[pickerId] = toValue;
            this.props.selectedValue[pickerId + 3] = selectedIndex;
